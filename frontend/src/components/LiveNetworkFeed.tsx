...
export const LiveNetworkFeed: React.FC<Props> = ({ contractAddress, rpcUrl }) => {
  ...
  const addEntry = useCallback((icon: string, text: string, color: string, txHash?: string) => {
    const now = new Date();
    const time = now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
    setEntries(prev => [...prev.slice(-100), { id: idRef.current++, timestamp: time, icon, text, txHash, color }]);
  }, []);
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (!connected) {
          setConnected(true);
          addEntry('🔌', 'Connected to 0G Galileo Testnet', 'text-emerald-400');
          addEntry('👁️', 'Watching for agent activity...', 'text-gray-500');
        }
        ...
        for (const e of creationEvents) {
          ...
          addEntry('🆕', `Agent "${name}" (#${args.agentId}) registered on-chain`, 'text-violet-400', e.transactionHash);
        ...
        for (const e of actionEvents) {
          ...
          addEntry('🤖', `${agentName} executed: ${actionType}`, 'text-cyan-400', e.transactionHash);
        ...
        for (const e of reviewEvents) {
          ...
          addEntry('📊', `${agentName} scored: ${score}/5`, color, e.transactionHash);
          setStats(s => ({ ...s, reviews: s.reviews + 1 }));
        }

      } catch (err) {
        if (connected) {
          addEntry('⚠️', `Poll error: ${(err as Error).message.slice(0, 60)}`, 'text-red-400');
        }
      }
    }
      ...
      <div className="bg-gray-950/80 px-4 py-3 font-mono text-[12px] leading-relaxed max-h-[400px] overflow-y-auto">
        ...
        {entries.map(e => (
          <div key={e.id} className="flex items-start gap-2 py-0.5 group hover:bg-gray-900/40 rounded px-1 -mx-1 transition-colors">
            <span className="w-5 text-center flex-shrink-0">{e.icon}</span>
