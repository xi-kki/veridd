...
describe('VeriddReputation', function () {
  ...
  it('Should reject name > 64 chars', async () => {
    ...
    await expect(veridd.createAgent(long, 'desc', 'uri')).to.be.revertedWith(
  ...
  it('Should reject desc > 500 chars', async () => {
    ...
    await expect(veridd.createAgent('Alpha', long, 'uri')).to.be.revertedWith(
  ...
  it('Should allow self-review (Zero Cup demo: judges can test full flow)', async () => {
    ...
    await expect(veridd.submitReview(0, 5, '0xroot', '0xroot', 'self')).to.not.be.reverted;
  ...
  it('Should reject invalid scores', async () => {
    ...
    const [, addr] = await ethers.getSigners();
    await expect(
      veridd.connect(addr).submitReview(0, 6, '0xroot', '0xroot', 'bad'),
